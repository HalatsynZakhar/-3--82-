...
for i in range(len(y[0])):
    y_average_list.append(average([y[0][i], y[1][i], y[2][i]], "y_average_{}row".format(i+1)))
...
for i in range(4):
    ...
    if abs(y_average_row_controls[i] - y_average_list[i]) >= 0.001:
        ...
        print("Yrow{} = {} + {}*{} + {}*{} + {}{} = {}\t\t\tY_average_{}row =  {}".format(i + 1, r(b0), par(b1),
                                                                                          par(x_factor_experiment[0][
                                                                                                  i]),
                                                                                          par(b2),
                                                                                          par(x_factor_experiment[1][
                                                                                                  i]),
                                                                                          par(b3),
                                                                                          par(x_factor_experiment[2][
                                                                                                  i]),
                                                                                          r(y_average_row_controls[i]),
                                                                                          i + 1,
                                                                                          r(y_average_list[i])))
...
print("f1=m-1=2 ; f2=N=4 Рівень значимості приймемо 0.05.")
N = 4
Gt = 0.7679 #табличне значення критерію Кохрена при N=4, f1=2, рівень значимості 0.05
printf("Gt", Gt)
if Gp <= Gt:
    Krit_Kohr = "Однор"
...
f3 = (m - 1) * N
t_tabl = 2.306 #табличне значення за критерієм Стюдента
...
print("Утворене рівняння регресіЇ: Y = {} + {}*X1 + {}*X2 + {}*X3".format(r(b_list[0]), par(b_list[1]), par(b_list[2]),
                                                                          par(b_list[3])))
...
for i in range(4):
    ...
    if abs(y_average_row_Student[i] - y_average_list[i]) >= 20:
        print(
            "Yrow{} = {} + {}*{} + {}*{} + {}*{} = \033[31m {}\t\t\t\033[0mY_average_{}row = \033[31m {}\033[0m".format(
                i + 1, r(b_list[0]), par(b_list[1]), par(x_factor_experiment[0][i]), par(b_list[2]),
                par(x_factor_experiment[1][i]), par(b_list[3]),
                par(x_factor_experiment[2][i]),
                r(y_average_row_Student[i]), i + 1, r(y_average_list[i])))
    elif abs(y_average_row_Student[i] - y_average_list[i]) >= 10:
        print("Yrow{} = {} + {}*{} + {}*{} + {}*{} = {}\t\t\tY_average_{}row =  {}".format(i + 1, r(b_list[0]),
                                                                                           par(b_list[1]),
                                                                                           par(x_factor_experiment[0][
                                                                                                   i]),
                                                                                           par(b_list[2]),
                                                                                           ...
                                                                                           par(x_factor_experiment[2][
                                                                                                   i]),
                                                                                           r(y_average_row_Student[i]),
                                                                                           i + 1,
                                                                                           r(y_average_list[i])))
        print("Результат приблизно (+-20) збігається! (Рівень значимості 0.05)")
    else:
        print("Yrow{} = {} + {}*{} + {}*{} + {}*{} = {}\t\t\tY_average_{}row =  {}".format(i + 1, r(b_list[0]),
                                                                                           par(b_list[1]),
                                                                                           par(x_factor_experiment[0][
...
print("\nТаблиця результату:")
print("|{: ^11}||{: ^11}||{: ^11}||{: ^11}|".format("x0_factor", "x1_factor", "x2_factor", "x3_factor"), end="")
print("|{: ^11}||{: ^11}||{: ^11}|".format("x1", "x2", "x3"), end="")
print("|{: ^11}||{: ^11}||{: ^11}||{: ^11}||{: ^11}||{: ^11}||{: ^11}||{: ^11}|"
      .format("y1", "y2", "y3", "Y_mid", "Y_mid_exp", "Стьюдент", "Krit_Kohr", "Krit_Fish"))
print("{:-^195}".format("-"))
for i in range(4):
    for j in range(4):
    ...
    print("|{: ^11}||{: ^11}||{: ^11}|".format(par(y_average_list[i]), par(y_average_row_controls[i]),
                                               par(y_average_row_Student[i])), end="")
    if i == 0:
